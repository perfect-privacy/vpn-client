...
class BuildMacos(BuildCommon):
    ...
    def _create_installer(self):
        ...
        cmd = 'cp -r "%s" "%s"' % (os.path.join(self.SOURCE_DIR, "build", "data", "macos", "installer", "scripts" ) ,  os.path.join(self.BUILD_DIR_TMP, "pkg"))
        os.system(cmd)
        cmd = 'cp -r "%s" "%s"' % (os.path.join(self.SOURCE_DIR, "build", "data", "macos", "installer", "resources" ) ,  os.path.join(self.BUILD_DIR_TMP, "pkg"))
        os.system(cmd)
        cmd = 'cp -r "%s" "%s/"' % (os.path.join(self.SOURCE_DIR, "build", "data", "macos", "installer", "distribution.xml" ) ,  os.path.join(self.BUILD_DIR_TMP, "pkg"))
        os.system(cmd)
        cmd = 'cp -r "%s" "%s/"' % (os.path.join(self.SOURCE_DIR, "build", "data", "macos", "installer", "Info.plist" ) ,  os.path.join(self.BUILD_DIR_TMP, "pkg", "app", "Applications", "Perfect Privacy.app", "Contents"))
        os.system(cmd)
        cmd = 'cp -a "%s"/* "%s"' % (os.path.join(self.SOURCE_DIR, "build_tmp", "perfect-privacy" ) , os.path.join(self.BUILD_DIR_TMP, "pkg", "app", "Applications", "Perfect Privacy.app", "Contents", "MacOS") )
        os.system(cmd)
