...
class StealthSSHCommon(StealthCommon):
    def __init__(self, core, servergroup, openvpn_remote_ip, openvpn_remote_port):
        ...
        if self._core.settings.stealth.stealth_custom_node.get() is True:
            self._ssh_host = self._core.settings.stealth.stealth_custom_hostname.get()
            self._ssh_port = self._core.settings.stealth.stealth_custom_port.get()
            self._ssh_fingerprint = self._core.settings.stealth.stealth_custom_ssh_fingerprint.get()
            if self._core.settings.stealth.stealth_custom_require_auth.get() is True:
                self._ssh_username = self._core.settings.stealth.stealth_custom_auth_username.get()
                self._ssh_password = self._core.settings.stealth.stealth_custom_auth_password.get()
        else:
            self._ssh_host = self._servergroup.vpn_server_config.ssh_ip
            self._ssh_fingerprint = self._servergroup.vpn_server_config.ssh_fingerprint
            self._ssh_port = self._core.settings.stealth.stealth_port.get()
            if self._ssh_port not in STEALTH_PORTS.ssh :
                self._ssh_port = random.choice(STEALTH_PORTS.ssh)

            self._ssh_username = self._core.settings.account.username.get()
            self._ssh_password = self._core.settings.account.password.get()

        self._ssh_local_port = "1111"
        self._process = None

        self.openvpn_arguments = []
        self.remote_host = "127.0.0.1"  # the ip or host the openvpn process actually connects to
        self.remote_port = self._ssh_local_port
        self.external_host_ip = self._ssh_host
        self.external_host_port = self._ssh_port
...
class SSHTunnelError(Exception):
    def __init__(self, msg, permission_denied=False):
        self.msg = str(msg)
...
class SSHTunnel(object):
    ...
    CONNECTION_STATE_UNSTABLE = 1
    CONNECTION_STATE_CONNECTED = 2
    CONNECTION_STATE_ERROR = 3

    CONNECTION_STATE_LIST = [
        CONNECTION_STATE_DISCONNECTED,
        CONNECTION_STATE_UNSTABLE,
